 ...
 *
 * @package Netflix-Theme
 */

namespace AssetsAudit\Traits;
...
trait Singleton {
	...
	final public static function get_instance() {
		...
		if ( ! isset( $instance[ $called_class ] ) ) {
			...
			/**
			 * Dependent items can use the `assets_audit_singleton_init_%s` hook to execute code
			 */
			do_action( sprintf( 'assets_audit_singleton_init_%s', $called_class ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores, WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
